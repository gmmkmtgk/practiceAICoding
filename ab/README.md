...
schedulr is a small Calendly-style booking service. A host posts the times they're free, a guest picks one of those open slots, and the app sends a confirmation email for the booking. The four endpoints that drive it are listed below.
...
- `POST /slots` — host creates an availability slot
- `GET /availability/<host>` — guest views a host's open slots
- `POST /book` — guest books a slot
- `GET /bookings/<host>` — host views their bookings
