...
fileshare is a small WeTransfer-style file sharing service. You upload a file and get back a share link with an expiry time and a maximum download count, and anyone with the link can download the file until it expires or the download limit runs out. The four endpoints that drive it are listed below.
...
- `POST /upload` — upload a file (multipart form: `file`, optional `expires_in_hours`, `max_downloads`)
- `GET /<token>` — download the file
- `GET /info/<token>` — view share metadata
- `DELETE /<token>` — delete the share
